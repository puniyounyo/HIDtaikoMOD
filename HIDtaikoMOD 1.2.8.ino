//　HIDtaikoMOD 1.2.8　NeoPixelを一時廃止
//　
//
//
//
...
void setup() {
  ...
  pinMode(16, INPUT_PULLUP);           // デジタル入力ピン16を入力プルアップモードに設定（通常HIGH）SW HOME
  ...
  display.println("ver.1.2.8");           // OLEDに"ver.1.2.8"という文字列を表示
...
//  updateLedColor(); // モードが変わった場合のみ LED を更新
  if (settingMode) {                       // 設定モードがtrueの場合
    if (digitalRead(4) == LOW) {         // デジタル入力ピン6がLOWの場合（数値減少ボタンが押された場合）
    ...
    if (digitalRead(5) == LOW) {         // デジタル入力ピン7がLOWの場合（数値増加ボタンが押された場合）
    ...
    if (swswitching == false) {         // PCモードの場合
      ...
      if (digitalRead(4) == LOW) {       // デジタル入力ピン4がLOWの場合
        SwitchControlLibrary().pressHatButton(Hat::UP);   // Switchの上ボタンを押す
        SwitchControlLibrary().sendReport();              // 押したボタンの状態を送信
        delay(de);                                        // デジタル入力遅延時間だけ待つ
        SwitchControlLibrary().releaseHatButton();        // Switchの上ボタンを離す
        SwitchControlLibrary().sendReport();                // 離したボタンの状態を送信
        delay(de);                                        // デジタル入力遅延時間だけ待つ
      }
      if (digitalRead(5) == LOW) {       // デジタル入力ピン5がLOWの場合
        SwitchControlLibrary().pressHatButton(Hat::DOWN); // Switchの下ボタンを押す
        SwitchControlLibrary().sendReport();              // 押したボタンの状態を送信
        delay(de);                                        // デジタル入力遅延時間だけ待つ
        SwitchControlLibrary().releaseHatButton();        // Switchの下ボタンを離す
        SwitchControlLibrary().sendReport();                // 離したボタンの状態を送信
        delay(de);                                        // デジタル入力遅延時間だけ待つ
      }
      if (digitalRead(6) == LOW) {       // デジタル入力ピン6がLOWの場合
        SwitchControlLibrary().pressHatButton(Hat::LEFT);  // Switchの左ボタンを押す
        SwitchControlLibrary().sendReport();              // 押したボタンの状態を送信
        delay(de);                                        // デジタル入力遅延時間だけ待つ
        SwitchControlLibrary().releaseHatButton();        // Switchの左ボタンを離す
        SwitchControlLibrary().sendReport();                // 離したボタンの状態を送信
        delay(de);                                        // デジタル入力遅延時間だけ待つ
      }
      if (digitalRead(7) == LOW) {       // デジタル入力ピン7がLOWの場合
        SwitchControlLibrary().pressHatButton(Hat::RIGHT); // Switchの右ボタンを押す
        SwitchControlLibrary().sendReport();              // 押したボタンの状態を送信
        delay(de);                                        // デジタル入力遅延時間だけ待つ
        SwitchControlLibrary().releaseHatButton();        // Switchの右ボタンを離す
        SwitchControlLibrary().sendReport();                // 離したボタンの状態を送信
        delay(de);                                        // デジタル入力遅延時間だけ待つ
      }
      if (digitalRead(8) == LOW) {       // デジタル入力ピン8がLOWの場合
      ...
      if (digitalRead(16) == LOW) {      // デジタル入力ピン16がLOWの場合
