...
void setup() {
  // ピンモードを直接指定
  pinMode(4, INPUT_PULLUP);            // デジタル入力ピン4を入力プルアップモードに設定（通常HIGH）SW ↑
  pinMode(5, INPUT_PULLUP);            // デジタル入力ピン5を入力プルアップモードに設定（通常HIGH）SW ↓
  pinMode(6, INPUT_PULLUP);            // デジタル入力ピン6を入力プルアップモードに設定（通常HIGH）SW ←
  pinMode(7, INPUT_PULLUP);            // デジタル入力ピン7を入力プルアップモードに設定（通常HIGH）SW →

  pinMode(8, INPUT_PULLUP);            // デジタル入力ピン8を入力プルアップモードに設定（通常HIGH）SW A
  pinMode(9, INPUT_PULLUP);            // デジタル入力ピン9を入力プルアップモードに設定（通常HIGH）SW B
  pinMode(0, INPUT_PULLUP);            // デジタル入力ピン0を入力プルアップモードに設定（通常HIGH）SW X
  pinMode(1, INPUT_PULLUP);            // デジタル入力ピン1を入力プルアップモードに設定（通常HIGH）SW Y

  pinMode(10, INPUT_PULLUP);           // デジタル入力ピン10を入力プルアップモードに設定（通常HIGH）SW MINUS
  pinMode(16, INPUT_PULLUP);           // デジタル入力ピン16を入力プルアップモードに設定（通常HIGH）SW PLUS
  ...
  display.println("ver.1.2.9");           // OLEDに"ver.1.2.9"という文字列を表示
...
//  updateLedColor(); // モードが変わった場合のみ LED を更新
  if (settingMode) {                       // 設定モードがtrueの場合
    ...
    if (swswitching == false) {         // PCモードの場合
      ...
      // --- Aボタン (Pin 8) / Bボタン (Pin 9) / Yボタン (Pin 1) / Xボタン (Pin 0) ---
      if (digitalRead(8) == LOW) SwitchControlLibrary().pressButton(Button::A); else SwitchControlLibrary().releaseButton(Button::A);
      if (digitalRead(9) == LOW) SwitchControlLibrary().pressButton(Button::B); else SwitchControlLibrary().releaseButton(Button::B);
      if (digitalRead(0) == LOW) SwitchControlLibrary().pressButton(Button::X); else SwitchControlLibrary().releaseButton(Button::X);
      if (digitalRead(1) == LOW) SwitchControlLibrary().pressButton(Button::Y); else SwitchControlLibrary().releaseButton(Button::Y);

      // --- システムボタンの合成ロジック (Pin 10 & 16) ---
      bool p10 = (digitalRead(10) == LOW); // MINUS物理ボタン
      bool p16 = (digitalRead(16) == LOW); // PLUS物理ボタン

      // いずれかのボタンが押された瞬間、時刻をメモする
      static bool lastAnyPressed = false;
      if ((p10 || p16) && !lastAnyPressed) {
        lastPressTime = millis();
      }
      lastAnyPressed = (p10 || p16);
      ...
      else if ((p10 || p16) && (millis() - lastPressTime > waitTime)) {
        // 【単体押し】どちらかが押されていて、かつ30ms経過しても相方が来ない場合
        SwitchControlLibrary().releaseButton(Button::HOME);
        if (p10) SwitchControlLibrary().pressButton(Button::MINUS); else SwitchControlLibrary().releaseButton(Button::MINUS);
        if (p16) SwitchControlLibrary().pressButton(Button::PLUS);  else SwitchControlLibrary().releaseButton(Button::PLUS);
      } 
      else if (!p10 && !p16) {
        // 【未入力】
        SwitchControlLibrary().releaseButton(Button::HOME);
