...
void setup() {
  ...
  display.println("ver.1.2.9b");           // OLEDに"ver.1.2.9b"という文字列を表示
...
void loop() {
  ...
  if (settingMode) {                       // 設定モードがtrueの場合
    ...
    if (swswitching == false) {         // PCモードの場合
      ...
      // --- Aボタン (Pin 7) / Bボタン (Pin 8) / Xボタン (Pin 6) / Yボタン (Pin 9) / Lボタン (Pin 0) / Rボタン (Pin 1)  ---
      if (digitalRead(7) == LOW) SwitchControlLibrary().pressButton(Button::A); else SwitchControlLibrary().releaseButton(Button::A);
      if (digitalRead(8) == LOW) SwitchControlLibrary().pressButton(Button::B); else SwitchControlLibrary().releaseButton(Button::B);
      if (digitalRead(6) == LOW) SwitchControlLibrary().pressButton(Button::X); else SwitchControlLibrary().releaseButton(Button::X);
      if (digitalRead(9) == LOW) SwitchControlLibrary().pressButton(Button::Y); else SwitchControlLibrary().releaseButton(Button::Y);
      if (digitalRead(0) == LOW) SwitchControlLibrary().pressButton(Button::L); else SwitchControlLibrary().releaseButton(Button::L);
      if (digitalRead(1) == LOW) SwitchControlLibrary().pressButton(Button::R); else SwitchControlLibrary().releaseButton(Button::R);



      // --- システムボタンの合成ロジック (Pin 14 & 15) ---
      bool p14 = (digitalRead(14) == LOW); // MINUS物理ボタン
      bool p15 = (digitalRead(15) == LOW); // PLUS物理ボタン

      // いずれかのボタンが押された瞬間、時刻をメモする
      static bool lastAnyPressed = false;
      if ((p14 || p15) && !lastAnyPressed) {
      ...
      if (p14 && p15) {
        ...
        // 【単体押し】どちらかが押されていて、かつ50ms経過しても相方が来ない場合
        SwitchControlLibrary().releaseButton(Button::HOME);
        if (p14) SwitchControlLibrary().pressButton(Button::MINUS); else SwitchControlLibrary().releaseButton(Button::MINUS);
        if (p15) SwitchControlLibrary().pressButton(Button::PLUS);  else SwitchControlLibrary().releaseButton(Button::PLUS);
      } 
      else if (!p14 && !p15) {
        // 【未入力】
        SwitchControlLibrary().releaseButton(Button::HOME);
