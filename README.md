...
このArduinoスケッチは、OLEDディスプレイを搭載した自作の太鼓コントローラーとして機能します。Pro Microをベースとしており、PCモードとNintendo Switchモードを切り替えることができ、それぞれ異なる入力方式で操作できます。設定モードでは、アナログセンサーの感度や各種遅延などのパラメータをOLED画面を見ながら調整し、EEPROMに保存できます。
